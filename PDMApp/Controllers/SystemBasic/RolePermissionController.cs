...
namespace PDMApp.Controllers
{
    ...
    public class RolePermissionController : ControllerBase
    {
        ...
        public RolePermissionController(IRolePermissionService roleService, ILogger<RolePermissionController> logger)
        {
        ...
        [HttpPost("initial")]
        public async Task<ActionResult<APIStatusResponse<IDictionary<string, object>>>> RolePageInitial()
        {
            try
            {
                var result = await _roleService.GetRolePageInitialAsync();
                return APIResponseHelper.HandleDynamicMultiPageResponse(result);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving role page initial data.");
        ...
        public async Task<ActionResult<APIStatusResponse<PagedResult<pdm_roles>>>> Roles([FromBody] RolesParameter value)
        {
            try
            {
                var result = await _roleService.GetRolesAsync(value);
                return APIResponseHelper.HandlePagedApiResponse(result); // 確保 result 是 PagedResult<pdm_roles>
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving roles.");
                return APIResponseHelper.HandleApiError<PagedResult<pdm_roles>>(
                    errorCode: "10002",
                    message: $"查詢過程中發生錯誤: {ex.Message}",
                    data: null
                );
        ...
        public async Task<ActionResult<APIStatusResponse<IDictionary<string, object>>>> RolePermissions([FromBody] PermissionsParameter value)
        {
            try
            {
                var result = await _roleService.GetRolePageInitialAsync(); // 假設這個方法可以回傳角色與權限
                return APIResponseHelper.HandleDynamicMultiPageResponse(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<APIStatusResponse<IDictionary<string, object>>>> UpsertRolePermission([FromBody] RolePermissionsParameter request)
        {
            try
            {
                var result = await _roleService.UpsertRoleAsync(request);
                return APIResponseHelper.HandleDynamicMultiPageResponse(result);
            }
            catch (Exception ex)
            {
