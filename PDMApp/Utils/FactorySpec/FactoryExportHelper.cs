...
namespace Utils.FactorySpec
{
    public static class FactoryExportHelper
    {
        public static byte[] ExportToExcel<T>(IEnumerable<T> data)
        {
            ...
            // 動態生成工作表，傳入查詢結果
            AddSheetToWorkbook(workbook, "Item", data, 1);
        ...
        private static void AddSheetToWorkbook<T>(XLWorkbook workbook, string sheetName, IEnumerable<T> data, int startRow)
        {
            ...
            // 添加表頭
            row = AddSheetHeader(worksheet, data, row);

            // 添加表身
            row = AddSheetBody(worksheet, data, row);
        ...
        private static int AddSheetHeader<T>(IXLWorksheet worksheet, IEnumerable<T> data, int startRow)
        {
            ...
            // 計算總頁數，假設每頁37筆
            int recordsPerPage = 37;
            int totalPages = (int)Math.Ceiling((double)dataCount / recordsPerPage);

            // 初始化頁碼變數
            string page = $"P.1/{totalPages}";
            ...
            worksheet.PageSetup.Margins.Bottom = 0.5; // 底邊距
            ...
            //{
            //    worksheet.Column(col).AdjustToContents(); // 根據內容調整寬度
            //}

            worksheet.PageSetup.AdjustTo(80); // 縮放比例 80%
        ...
        private static int AddSheetBody<T>(IXLWorksheet worksheet, IEnumerable<T> data, int startRow)
        {
            int row = startRow;
            ...
            foreach (var item in specificData)
            {
                ...
                for (int col = 1; col <= 13; col++) // 遍歷每一列
                {
                    ...
                    // 針對未特殊處理的欄位設置預設字體大小
                    if (col != 2 && col != 4 && col != 9 && col != 10 && col != 13) // 跳過已特殊處理的欄位
                    {
        ...
        private static int AddSheetFooter<T>(IXLWorksheet worksheet, IEnumerable<T> data, int startRow)
        {
            int row = startRow;

            // 添加一些表尾範例數據
            worksheet.Cell(row, 1).Value = "表尾範例";
