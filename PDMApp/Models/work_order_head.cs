using System;
using System.Collections.Generic;

#nullable disable

namespace PDMApp.Models
{
    public partial class work_order_head
    {
        public string wk_m_id { get; set; }
        public string fact_no { get; set; }
        public string brand_no { get; set; }
        public string order_no { get; set; }
        public decimal? order_ver { get; set; }
        public string purpose { get; set; }
        public string season { get; set; }
        public string dev_no { get; set; }
        public string stage { get; set; }
        public string article_no { get; set; }
        public string colorway { get; set; }
        public string matlway { get; set; }
        public string model_nm { get; set; }
        public string style_no { get; set; }
        public string color_no { get; set; }
        public string category { get; set; }
        public string gender { get; set; }
        public string mold_no { get; set; }
        public string last_no { get; set; }
        public string last_width { get; set; }
        public DateTime? req_date { get; set; }
        public string type_definition { get; set; }
        public string b_pm { get; set; }
        public string sample_size { get; set; }
        public string txt_attrib_01 { get; set; }
        public string txt_attrib_02 { get; set; }
        public string txt_attrib_03 { get; set; }
        public string txt_attrib_04 { get; set; }
        public string txt_attrib_05 { get; set; }
        public string txt_attrib_06 { get; set; }
        public string txt_attrib_07 { get; set; }
        public string txt_attrib_08 { get; set; }
        public string txt_attrib_09 { get; set; }
        public string txt_attrib_10 { get; set; }
        public string txt_attrib_11 { get; set; }
        public string txt_attrib_12 { get; set; }
        public string txt_attrib_13 { get; set; }
        public string txt_attrib_14 { get; set; }
        public string txt_attrib_15 { get; set; }
        public string txt_attrib_16 { get; set; }
        public string txt_attrib_17 { get; set; }
        public string txt_attrib_18 { get; set; }
        public string txt_attrib_19 { get; set; }
        public string txt_attrib_20 { get; set; }
        public decimal create_user { get; set; }
        public DateTime create_time { get; set; }
        public decimal modify_user { get; set; }
        public DateTime modify_time { get; set; }
        public char del_mk { get; set; }
        public DateTime? del_date { get; set; }
        public char? proc_mk { get; set; }
        public string err_msg { get; set; }
        public string style_id { get; set; }
        public string order_type { get; set; }
        public string plm_spec_m_id { get; set; }
        public string order_status { get; set; }
        public string trans_id { get; set; }
        public string cust_sr { get; set; }
        public string report_sort { get; set; }
        public string order_kind { get; set; }
        public string fg_type { get; set; }
        public string spec_m_id { get; set; }
        public string wk_memo { get; set; }
        public string step_memo { get; set; }
        public long? serp_req_date { get; set; }
        public char? pl_mk { get; set; }
    }
}
